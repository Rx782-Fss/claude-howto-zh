...
|  | `测试-engineer.md` | 测试覆盖分析 | read, write, bash, grep | 测试自动化 |  |
|  | `文档-writer.md` | 文档创建 | read, write, grep | 文档生成 |  |
|  | `secure-reviewer.md` | 安全审查（只读） | read, grep | 安全审计 |  |
|  | `implementation-代理.md` | 完整实现 | read, write, bash, grep, edit, glob | 功能开发 |  |
...
|  | 文件 | 说明 | 集成对象 | 使用场景 |  |
|  | ------ | ------ | ---------- | ---------- |  |
|  | `github-mcp.JSON` | GitHub 集成 | GitHub API | PR / 问题 管理 |  |
...
|  | 文件 | 说明 | 事件 | 使用场景 |  |
|  | ------ | ------ | ------ | ---------- |  |
|  | `格式-code.sh` | 自动格式化代码 | `PreToolUse:Write` | 代码格式化 |  |
|  | `pre-提交.sh` | 提交前运行测试 | `PreToolUse:Bash` | 测试自动化 |  |
|  | `安全性-scan.sh` | 安全扫描 | `PostToolUse:Write` | 安全检查 |  |
|  | `log-bash.sh` | 记录 bash 命令 | `PostToolUse:Bash` | 命令日志 |  |
|  | `validate-prompt.sh` | 验证提示词 | `PreToolUse` | 输入校验 |  |
|  | `notify-团队.sh` | 发送通知 | `Notification` | 团队通知 |  |
...
**Hook 类型**（4 类，25 个事件）：
- 工具 钩子：`PreToolUse`、`PostToolUse`、`PostToolUseFailure`、`PermissionRequest`
- 会话 钩子：`SessionStart`、`SessionEnd`、`停止`、`StopFailure`、`SubagentStart`、`SubagentStop`
- 任务 钩子：`UserPromptSubmit`、`TaskCompleted`、`TaskCreated`、`TeammateIdle`
- 生命周期 钩子：`ConfigChange`、`CwdChanged`、`FileChanged`、`PreCompact`、`PostCompact`、`WorktreeCreate`、`WorktreeRemove`、`Notification`、`InstructionsLoaded`、`Elicitation`、`ElicitationResult`
...
|  | 文件 | 说明 | 内容 |  |
|  | ------ | ------ | ------ |  |
|  | `README.md` | 文档 | 完整的 检查点 指南 |  |
|  | `检查点-示例.md` | 真实示例 | 数据库迁移、性能优化、UI 迭代、调试 |  |
...
|  | `config-示例.JSON` | 配置示例 | 10+ 个场景化配置 |  |
|  | `planning-mode-示例.md` | 规划示例 | REST API、数据库迁移、重构 |  |

**扩展功能**：
- Scheduled Tasks：使用 `/loop` 和 cron 工具的周期任务
- Chrome 集成：通过无头 Chromium 做浏览器自动化
...
- `claude --model` - 选择模型（Sonnet 4.6、Opus 4.7）
...
|  | `README.md` | `/02-记忆/` | 记忆 指南 |  |
|  | `README.md` | `/03-技能/` | 技能 指南 |  |
|  | `README.md` | `/04-子代理（子代理）/` | 子代理（子代理） 指南 |  |
|  | `README.md` | `/05-mcp/` | MCP 指南 |  |
|  | `README.md` | `/06-钩子/` | 钩子 指南 |  |
|  | `README.md` | `/07-插件/` | 插件 指南 |  |
|  | `README.md` | `/08-checkpoints/` | Checkpoints 指南 |  |
|  | `README.md` | `/09-advanced-features/` | 高级功能 指南 |  |
...
#  在 settings 中配置 钩子（~/.claude/settings.JSON）
#  参见 06-钩子/README.md
```
...
```bash
#  配置高级功能
#  参见 09-advanced-features/config-示例.JSON
...
|  | 分类 | Commands | Agents | MCP | 钩子 | Scripts | Templates | Docs | Images | 总计 |  |
|  | ------ | ---------- | -------- | ----- | ------- | --------- | ----------- | ------ | -------- | ------ |  |
...
- `01-slash-commands/优化.md` - 性能分析
- `04-子代理（子代理）/code-reviewer.md` - 性能审查
- `03-技能/code-review/` - 性能指标
- `07-插件/pr-review/agents/性能-analyzer.md` - 性能专家

### 安全

- `04-子代理（子代理）/secure-reviewer.md` - 安全审查
- `03-技能/code-review/` - 安全分析
- `07-插件/pr-review/` - 安全检查

### 测试

- `04-子代理（子代理）/测试-engineer.md` - 测试工程师
- `07-插件/pr-review/commands/check-tests.md` - 测试覆盖率

### 文档

- `01-slash-commands/generate-API-docs.md` - API 文档命令
- `04-子代理（子代理）/文档-writer.md` - 文档编写 代理
- `03-技能/doc-generator/` - 文档生成 技能
- `07-插件/文档/` - 完整文档插件

### 部署

- `07-插件/devops-automation/` - 完整 DevOps 方案

### 自动化

- `06-钩子/` - 事件驱动自动化
- `06-钩子/pre-提交.sh` - 提交前自动化
- `06-钩子/格式-code.sh` - 自动格式化
- `09-advanced-features/` - headless 模式用于 CI/CD

### 校验

- `06-钩子/安全性-scan.sh` - 安全校验
- `06-钩子/validate-prompt.sh` - 提示词校验

### 实验

- `08-checkpoints/` - 使用 rewind 做安全实验
- `08-checkpoints/检查点-示例.md` - 真实示例

### 规划

- `09-advanced-features/planning-mode-示例.md` - 规划模式示例
- `09-advanced-features/README.md` - 深度思考

### 配置

- `09-advanced-features/config-示例.JSON` - 配置示例
...
- https://支持.claude.com/en/articles/12138966-发布-notes
