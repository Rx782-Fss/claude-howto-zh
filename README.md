...
- **GitHub stars**——来自日常使用 Claude Code 的开发者们
- **Forks**——各团队正在将此指南适配到自己的工作流中
- **积极维护**——与每次 Claude Code 发布同步更新（最新：v2.1.119，2026 年 4 月）
...
> **安装说明**: 从 v2.1.113 开始,Claude Code 以原生平台特定二进制文件形式发布(macOS/Linux/Windows)。`npm install -g @anthropic-ai/claude-code` 仍然可用——原生二进制文件会在首次使用时作为可选依赖下载。从 v2.1.116 开始,下载来源为 `https://downloads.claude.ai/claude-code-releases` ——企业代理必须将该主机添加到白名单。
...
**这个项目还在维护吗?**
是的,积极维护中。本指南与每次 Claude Code 发布同步更新。当前版本:v2.1.119(2026 年 4 月),兼容 Claude Code 2.1+。
...
可以。所有模板均兼容 Claude Sonnet 4.6、Claude Opus 4.7 和 Claude Haiku 4.5。
