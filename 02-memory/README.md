...
```mermaid
graph TD
    A["托管策略 /Library/.../ClaudeCode/CLAUDE.md"] -->|最高优先级| A2["托管扩展配置 managed-settings.d/"]
    A2 --> B["项目记忆 ./CLAUDE.md"]
    B --> C["项目规则 ./.claude/rules/*.md"]
    C --> D["用户记忆 ~/.claude/CLAUDE.md"]
    D --> E["用户规则 ~/.claude/rules/*.md"]
    E --> F["本地项目记忆 ./CLAUDE.local.md"]
    F --> G["自动记忆 ~/.claude/projects/.../memory/"]
...
```mermaid
graph TD
    A["Claude 会话开始"] --> B["加载 MEMORY.md (前200行)"]
    B --> C["会话活跃中"]
    C --> D["Claude 发现 模式与洞察"]
    D --> E["写入 自动记忆"]
    E -->|通用笔记| F["MEMORY.md"]
